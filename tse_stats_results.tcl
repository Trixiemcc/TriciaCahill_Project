...
set f [open "TestResults" w+];

puts $f "Info: Opened JTAG Master Service\n";

puts $f "TSE MAC Statistics Counters Map";
puts $f "==============================";
puts $f "Addr\t Name\t\t\t\t Read Value";
puts $f "----\t ----\t\t\t\t ----------";
...
puts $f "$AddressOffset_hex\t $mac_register_name\t [format %u $mac_register_read_value]";
